...
Every data row must contain two finite, strictly positive numeric values. Rows may appear in any order; they are sorted by energy. Duplicate energies are rejected.
...
- `Makefile`: `make test`, `make generate`, `make pdf`, `make artifacts` targets.
